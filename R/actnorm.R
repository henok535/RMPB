...
#' actnorm : The probability of an event when all subjects are assigned to the active treatment
#'
#' @description This function is used to estimate the probability of an event assuming all the subjects are
#'              are assigned the treatment group. The predictive biomarker is assumed
...
#'   eventactive <- actnorm(coff, dpar=c(2.3,0.2))
#' @author Henok Woldu
#' @export


actnorm <- function(coeff, dpar) {
                     myreturn=0
       ...
       myreturn=simpson_v2(fun1, -Inf, Inf, n=1000)
